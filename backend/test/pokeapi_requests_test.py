...
#     'fkt',
#     [(pokeapi_requests.get_all_pokemon), (pokeapi_requests.get_all_types)],
# )
...
#     assert pokeapi_requests.get_all_types() == {'results': [{'name': 'water', 'url': 'ignore'}, {'name': 'steel'}]}
...
#     assert pokeapi_requests.get_pokemon_of_type('water') == {
#             'pokemon': [{'pokemon': {'name': 'wartortle', 'url': 'ignore'}, 'slot': 1}, {'pokemon': {'name': 'seadra'}}]
#         }
#     mock_requests.get.assert_called_with(f'{config.POKEAPI_TYPE_URL}/water/{config.HIGH_LIMIT}')
